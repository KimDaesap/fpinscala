...
object List {
  ...
  def hasSubsequence[A](sup: List[A], sub: List[A]): Boolean = {
    def startWith[A](list: List[A], sub: List[A]): Boolean = {
      (list, sub) match {
        case (_, Nil) => false
        case (Cons(lh, lt), Cons(sh, st)) if lh == sh => startWith(lt, st)
        case _ => false
      }
    }
    ...
    (sup, sub) match {
      case (Nil, bs) => bs == Nil
      case (as, bs) if startWith(as, bs) => true
      case (Cons(ah,at), Cons(bh,bt)) => hasSubsequence(at, bt)
    }
  }
